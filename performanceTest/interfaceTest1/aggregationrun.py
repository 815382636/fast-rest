...
start_step = 0
param_file = "bivariate_aggregation.txt"
...
if start_step <= 2:
    ...
    for i in range(0, n):
        ...
        # 2.绘制综合JSON图像
        print("# 2.绘制第%d幅综合折线图像..." % (i + 1))
        line_chart = name + line_postfix
        area_chart = name + area_postfix

        color = "#4FC1E8"
        alt.Chart(json_data).transform_fold(value_labels).mark_line().encode(
            alt.X(scale=alt.Scale(domain=[start_time, end_time]), field=time_label, type="temporal",
                  axis=alt.Axis(title="", labelFontSize=20)),
            alt.Y(scale=alt.Scale(domain=[min_value, max_value]), field="value", type="quantitative",
                  axis=alt.Axis(title="", labelFontSize=20)),
            color="key:N"
        ).properties(
            width=width,
        ...
        # print("# 2.绘制第%d幅综合面积图像..." % (i + 1))
        #
        # alt.Chart(json_data).transform_fold(value_labels).mark_area().encode(
        #     alt.X(scale=alt.Scale(domain=[start_time, end_time]), field=time_label, type="temporal",
        #           axis=alt.Axis(title="", labelFontSize=20)),
        #     alt.Y(scale=alt.Scale(domain=[min_value, max_value]), field="value", type="quantitative",
        #           axis=alt.Axis(title="", labelFontSize=20)),
...
if start_step <= 3:
    ...
    msssim_result = open("msssim_result.txt", "a")
    # msssim_result.write("%f,%f\n" % (percent, alpha))
    # msssim_result.write("sample,msssim\n")
    for area in area_dir_list:
        msssim_result.write(area+"\n")
        area_charts = os.listdir(area)
        for area_chart in area_charts:
            msssim = calcMSSSIM(area + "/" + base_area_png, area + "/" + area_chart)
...
if start_step <= 4:
    ...
    for value_label in value_labels:
        os.system("rm -R %s" % (value_label+"_"+pixel_dir))
        os.system("mkdir -p %s" % (value_label+"_"+pixel_dir))
        pixel_dir_list.append(value_label+"_"+pixel_dir)

    pae_result = open("pae_result.txt", "a")
    pae_result.write("sample,pae\n")
    for line_dir in line_dir_list:
        if line_dir == "line_png":
            continue
        pae_result.write(line_dir+"\n")
        ...
        for line_chart in line_charts:
            ...
            os.system("mv *%s %s" % (pixel_postfix, line_dir.split("_")[0]+"_"+pixel_dir))
...
if start_step <= 5:
    ...
    for area_dir in area_dir_list:
        if area_dir == "area_png":
            continue
        mse_result.write(area_dir+"\n")
        ...
        for area_chart in area_charts:
            ...
            os.system("mv *%s %s" % (pixel_postfix, area_dir.split("_")[0]+"_"+pixel_dir))
...
if start_step <= 6:
    ...
    mse_result = open("pmse_result.txt", "a")
    mse_result.write("sample,pmse\n")
    for line_dir in line_dir_list:
        if line_dir =="line_png":
            continue
        mse_result.write(line_dir+"\n")
