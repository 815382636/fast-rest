...
m4_fast =[1,2,3,4,5]
sample_fast =[1,2,3,4,5]
aggregation_fast =[1,2,3,4,5]
outlier =[1,2,3,4,5]
weight =[1,2,3,4,5]
...
for i in range(1, 16):
    ...
    for line in lines:
        ...
        if "fast" in line:
            if "random" in line:
                sample_fast[index] = float(line.split(",")[1])
            if "m4" in line:
                m4_fast[index] = float(line.split(",")[1])
            if "aggregation" in line:
                aggregation_fast[index] = float(line.split(",")[1])
            if "outlier" in line:
    ...
    plt.plot(account, m4_fast, "o-", color='#AC92EB', label="m4-fast")
    plt.plot(account, sample_fast, "o-", color='#FFCE54', label="sample-fast")
    plt.plot(account, aggregation_fast, "o-", color='#0870CB', label="agg-fast")
    plt.plot(account, outlier, "o-", color='#A0D568', label="outlier-fast")
    plt.plot(account, weight, "o-", color='#D20338', label="weight-fast")
    plt.plot(account, m4, "v-.", color='#AC92EB', label="m4")
    plt.plot(account, sample, "v-.", color='#FFCE54', label="sample")
    plt.plot(account, aggregation, "v-.", color='#0870CB', label="agg")
...
ax.legend(bbox_to_anchor=(-0.35, -0.4), loc='lower right', ncol=7, fontsize="x-large")
