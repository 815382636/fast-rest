...
public class ErrorController {
	...
	public static void lineError(List<Map<String, Object>> data, List<Map<String, Object>> sample, String label,
		...
		for (int i = 1; i < sample.size(); i++) {
			...
			if (value instanceof Double)
				...
				b0 = ((Integer) value).doubleValue();
			else if (value instanceof Long)
				b0 = ((Long) value).doubleValue();
			else
				b0 = (Double) value;
			...
			if (value instanceof Double)
				...
				b1 = ((Integer) value).doubleValue();
			else if (value instanceof Long)
				b1 = ((Long) value).doubleValue();
			else
				b1 = (Double) value;

			// 找到每两个样本点之间对应的原始数据
			for (newIndex = lastIndex; newIndex < data.size(); newIndex++) {
				if ((long) data.get(newIndex).get("timestamp") > a1L)
					break;
			}
			...
			for (int j = lastIndex; j < newIndex; j++) {
				...
				if (value instanceof Double)
					...
					y0 = ((Integer) value).doubleValue();
				else if (value instanceof Long)
					y0 = ((Long) value).doubleValue();
				else
					y0 = (Double) value;
				...
				if (value instanceof Double)
					...
					y1 = ((Integer) value).doubleValue();
				else if (value instanceof Long)
					y1 = ((Long) value).doubleValue();
				else
					y1 = (Double) value;
	...
	public static void correlation_lineError(List<Map<String, Object>> data, List<Map<String, Object>> sample,
			String[] labels, boolean isRawData) {
		if (sample.size() < 1)
			return;
//        sample.get(0).put("error", 0.0);
//        sample.get(0).put("area", 0.0);
		for (int i = 0; i < labels.length; i++) {
			sample.get(0).put(labels[i] + "error", 0.0);
			sample.get(0).put(labels[i] + "area", 0.0);
		}
		if (sample.size() == 1)
			return;
			...
			for (String label : labels) {
				...
				for (int j = lastIndex; j < newIndex; j++) {
					...
					if (isRawData) {
						...
						area += (Double) data.get(j).get(label+"area");
						// 除L0层级外，更高层级样本误差需累加已有层级样本的误差
						error += (Double) data.get(j).get(label+"error");
					}
				...
				if (Double.isNaN(area))
					area = 0.0;
				sample.get(i).put(label + "error", error);
				sample.get(i).put(label + "area", area);
